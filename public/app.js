...
async function bootstrapConversation() {
  ...
  const params = new URLSearchParams({ action: "bootstrap", mac_address: macAddress });
  if (currentConversationId) params.set("conversation_id", String(currentConversationId));
  const response = await fetch(`${SUPABASE_FUNCTION_URL}?${params.toString()}`, { headers: authHeaders(), cache: "no-store" });
